  ...
  computed: {
    ...
    progress: function () {
      ...
      if (this.mode === "mDesign") {
        ...
        if (this.totalTime < 0) {
          return math.parse(this.presTime).toString(4) + " / " + outStr;
        } else {
          return math.parse(this.presTime).toString(4) + " / " + outStr + " = " + math.parse(this.presTime / this.totalTime*100).toString(4) + "%";
        }
      }
    }
  },
