class umk_1614794258460 extends umk_model {
  Icon() {
    return {
  ...
  Evaluate() {
    if (this.Parameters.iSelected.Value[0][0] === "ae") {
      this.outputs[0] = math.matrix([
        [math.variance(this.inputs[0], this.Parameters.normalization.Value[0][0])]
      ]);
    } else if (this.Parameters.iSelected.Value[0][0] === "rw") {
      this.outputs[0] = math.transpose(math.matrix([math.variance(this.inputs[0], 1, this.Parameters.normalization.Value[0][0])]));
    } else {
      this.outputs[0] = math.matrix([math.variance(this.inputs[0], 0, this.Parameters.normalization.Value[0][0])]);
    }
  ...
  Details(short = false) {
    ...
    let outString_2 = "";
    if (this.Parameters.iSelected.Value[0][0] === "ae") {
      outString_1 = "\\sigma^2 (u)"; //TeX.prepDisp();
      outString_2 = "y=";
    } else if (this.Parameters.iSelected.Value[0][0] === "rw") {
      outString_1 = "\\sigma^2 (u_{i,\\left[1,n\\right]})"; //TeX.prepDisp();
      outString_2 = "y_{i,1}=";
    } else {
      outString_1 = "\\sigma^2 (u_{\\left[1,m\\right],j})"; //TeX.prepDisp();
      outString_2 = "y_{1,j}=";
    }
    if (short) {
