class umk_1614793980339 extends umk_model {
    Icon() {
      return {
    ...
    Evaluate() {
      if (this.Parameters.iSelected.Value[0][0] === "ae") {
        this.outputs[0] = math.matrix([
          [math.min(this.inputs[0])]
        ]);
      } else if (this.Parameters.iSelected.Value[0][0] === "rw") {
        this.outputs[0] = math.transpose(math.matrix([math.min(this.inputs[0], 1)]));
      } else {
        this.outputs[0] = math.matrix([math.min(this.inputs[0], 0)]);
      }
    ...
    Details(short = false) {
      ...
      let outString_2 = "";
      if (this.Parameters.iSelected.Value[0][0] === "ae") {
        outString_1 = "\\min (u)"; //TeX.prepDisp();
        outString_2 = "y=";
      } else if (this.Parameters.iSelected.Value[0][0] === "rw") {
        outString_1 = " \\min \\left( u_{i,\\left[1,n\\right]}\\right)"; //TeX.prepDisp();
        outString_2 = "y_{i,1}=";
      } else {
        outString_1 = "\\min \\left( u_{\\left[1,m\\right],j}\\right)"; //TeX.prepDisp();
        outString_2 = "y_{1,j}=";
      }
      if (short) {
