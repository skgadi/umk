class umk_1774714197073 extends umk_model {
  ...
  Evaluate() {
    ...
    this.portDetails.sendValue = this.getRightValueToOutput(sendValue);
  }
  ...
  invalidParams() {
    ...
    if (
      this.Parameters.freq.Value[0][0] <= 0 ||
      this.Parameters.freq.Value[0][0] > 40000000
    ) {
    ...
    const resolutionBits = Math.floor(
      Math.log2(80000000 / this.Parameters.freq.Value[0][0]),
    );
    if (resolutionBits < 1) {
    ...
    if (
      this.Parameters.res.Value[0][0] <= 0 ||
      this.Parameters.res.Value[0][0] > 16
    ) {
