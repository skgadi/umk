class umk_1614793807400 extends umk_model {
  Icon() {
    return {
  ...
  Evaluate() {
    if (this.Parameters.iSelected.Value[0][0] === "ae") {
      this.outputs[0] = math.matrix([
        [math.prod(this.inputs[0])]
      ]);
    } else if (this.Parameters.iSelected.Value[0][0] === "rw") {
      let gSize = this.inputs[0].size();
      const TempOutArray = [];
      for (let i = 0; i < gSize[0]; i++) {
        let TempResult = math.prod(this.inputs[0].subset(math.index(i, Array.from(Array(gSize[1]).keys()))));
      ...
      for (let i = 0; i < gSize[1]; i++) {
        let TempResult = math.prod(this.inputs[0].subset(math.index(Array.from(Array(gSize[0]).keys()), i)));
        TempOutArray.push(TempResult);
      }
      this.outputs[0] = math.matrix([TempOutArray]);
    }
  ...
  Details(short = false) {
    ...
    let outString_2 = "";
    if (this.Parameters.iSelected.Value[0][0] === "ae") {
      outString_1 = "\\prod_{i=1}^{m} \\prod_{j=1}^{n} \\left( u_{i,j}\\right)"; //TeX.prepDisp();
      outString_2 = "y=";
    } else if (this.Parameters.iSelected.Value[0][0] === "rw") {
      outString_1 = " \\prod_{j=1}^{n} \\left( u_{i,j}\\right)"; //TeX.prepDisp();
      outString_2 = "y_{i,1}=";
    } else {
      outString_1 = "\\prod_{i=1}^{m} \\left( u_{i,j}\\right)"; //TeX.prepDisp();
      outString_2 = "y_{1,j}=";
    }
    if (short) {
