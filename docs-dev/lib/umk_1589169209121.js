class umk_1589169209121 extends umk_model {
    Icon() {
      return {
    ...
    Evaluate() {
      this.outputs[0] = math.xor(this.inputs[0], this.inputs[1]);
      //console.log(this.TerminalsIn.value);
      for (let i = 2; i < this.TerminalsIn.value; i++) {
        this.outputs[0] = math.xor(this.outputs[0], this.inputs[i]);
      }
    }
    Details() {
      let outArr = [];
      ...
      for (let i = 0; i < outArr.length; i++) {
        ...
        for (let j = 1; j < outArr[i].length; j++) {
          tempVal = math.xor(tempVal, outArr[i][j]);
        }
