class umk_1614796116806 extends umk_model {
    Icon() {
      return {
    ...
    Evaluate() {
      if (this.Parameters.iSelected.Value[0][0] === "ae") {
        ...
        for (let i = 0; i < gSize[0]; i++) {
          let TempResult = math.hypot(this.inputs[0].subset(math.index(i, Array.from(Array(gSize[1]).keys()))));
        ...
        for (let i = 0; i < gSize[1]; i++) {
          let TempResult = math.hypot(this.inputs[0].subset(math.index(Array.from(Array(gSize[0]).keys()), i)));
          TempOutArray.push(TempResult);
        }
        this.outputs[0] = math.matrix([TempOutArray]);
      }
