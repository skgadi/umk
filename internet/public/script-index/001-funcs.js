...
function openmodal(evt) {
  for (let i = 0; i < evt.path.length; i++) {
    if (!!evt.path[i].className && evt.path[i].tagName === "DIV" && !!evt.path[i].nextSibling && !!evt.path[i].nextSibling.className && evt.path[i].nextSibling.className.indexOf("w3-modal")>=0) {
