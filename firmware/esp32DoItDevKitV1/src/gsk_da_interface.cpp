        ...
        try {
            ...
            if (element.is_object()) {
                ...
                if (element["type"] == "DO") {
                    int pins[1];
                    pins[0] = element["pin"];
                    GSK_IO_DIGITAL_OUT* io =
                        new GSK_IO_DIGITAL_OUT(i, pins, element["initVal"]);
                    mainPropertiesList->addByRewrite((GSK_IO_PROPERTIES*)io);
                }
                if (element["type"] == "AI") {
                    int pins[1];
                ...
                if (element["type"] == "PM") {
                    int pins[1];
                    pins[0] = element["pin"];
                    GSK_IO_PWM* io = new GSK_IO_PWM(
                        i, pins, element["initVal"], element["ch"],
                        element["freq"], element["res"]);
                    mainPropertiesList->addByRewrite((GSK_IO_PROPERTIES*)io);
                }
                if (element["type"] == "EN") {
                    int pins[2];
...
void GSK_DA_INTERFACE::processLoop() { mainPropertiesList->loop(); }